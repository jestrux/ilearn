...
const MultiRangeSlider = (props) => {
	...
	const [minValue, set_minValue] = useState(parseFloat(props.minValue != undefined ? props.minValue : 25));
	const [maxValue, set_maxValue] = useState(parseFloat(props.maxValue != undefined ? props.maxValue : 75));
	...
	useEffect(() => {
		FirstTimeUseEffect &&
			(() => {
                const wheel = document.querySelector('#' + ID);
                if(wheel){
	...
	return (
		<div id={ID} className={baseClassName} onWheel={onMouseWheel}>
			<div className='bar'>
				...
				<div className='bar-inner bg-gradient-to-r from-blue-700 to-indigo-500'>
